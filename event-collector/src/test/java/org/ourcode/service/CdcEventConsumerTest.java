...
class CdcEventConsumerTest {
    ...
    void testHandleCdcEvents() {
        ...
        verify(outBoxService, times(1)).markAsProcessed(anyList());
        verify(deviceIdProducer, times(1)).sendProcessedDeviceIds(anyList());
    }

    @Test
    void testHandleCdcEventsWithNullAfter() {
        ...
        verify(deviceIdProducer, never()).sendProcessedDeviceIds(anyList());
    }
}
